...
class CharLife:
    ...
    def __init__(self, level: str):
        ...
        self.level = __class__.LEVEL[level]  # オブジェクトLEVELに応じた数字(life)
    ...
    while True:
        ...
        for bomb in pg.sprite.spritecollide(bird, bombs, True):
            if bird.state == "hyper":  # hyperモードの時
                ...
                実装：機能５
                """
            elif char_life.level > 1:  # 残機が残っているとき
                exps.add(Explosion(bomb, 50))
                char_life.life_kill()  # 残機を１減らす
            else:  # normalモードの時
                bird.change_img(8, screen)  # こうかとん悲しみエフェクト
                score.update(screen)
                pg.display.update()
        ...
        shields.draw(screen)
        score.update(screen)
        char_life.update(screen)  # 実装：機能５
        pg.display.update()
        tmr += 1
        clock.tick(50)
